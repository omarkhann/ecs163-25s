...
// NOTE: For the Sankey diagram, one person can have multiple mental health conditions, so the flow out of the "Year" bars will be larger than the flow in. I was unsure how to implement this in the sankey diagram, since it would mean the flows overlap. I am unsure if this makes the sankey a poor fit or is an acceptable characteristic.

let original_data_bar_chart = [];

d3.csv("data/Student Mental health.csv").then(data => {
    original_data_bar_chart = data;
...
document.addEventListener("DOMContentLoaded", function() {
    d3.selectAll('#bar-chart-filters input').on('change', update_bar_chart);
}); // update the bar chart when the selected filters change
...
// helper to get selected categories from checkboxes
function get_selected_category() {
    return Array.from(document.querySelectorAll('#bar-chart-filters input:checked')).map(cb => cb.value);
}

// main function to filter and update the bar chart
function update_bar_chart() {
    const selected_categories = get_selected_category();
    create_bar_chart(original_data_bar_chart, selected_categories);
}


// make a bar chart to depict the spread of each mental health option by gender
function create_bar_chart(data, selected_categories = mental_health_categories) {
   ...
   // count occurrences of each mental health category per gender
    const grouped_data = {};
    data.forEach(d => {
        ...
        mental_health_categories.slice(0, -1).forEach(category => {
            ...
            if (response === "Yes") {
                ...
                grouped_data[key] = (grouped_data[key] || 0) + 1;
            }
        });
        if (!has_conditions) {
    ...
    // prepare array with percentages based on total per gender
    ...
    // only display selected categories, but keep their percent values
    const display_data = grouped_data_array.filter(d => selected_categories.includes(d.category));

    // set up scales
    const x0_scale = d3.scaleBand()
        .domain([...new Set(display_data.map(d => d.gender))])
    ...
    const gender_groups = svg.append("g")
        .selectAll("g")
        .data(d3.group(display_data, d => d.gender))
        .join("g")
        .attr("transform", d => `translate(${x0_scale(d[0])},0)`);

    gender_groups.selectAll("rect")
        .data(d => d[1])
        .join(
            enter => enter.append("rect")
                .attr("x", d => x1_scale(d.category))
...
function create_sankey(data) {
    ...
    const node = svg.append("g")
        ...
        .on("mouseover", function(event, d) {
            ...
            // calculate total value for this node (sum of incoming and outgoing links)
             const total = links
            ...
            d3.select("#sankey-tooltip")
                .style("display", "block")
                .html(`<strong>${d.name}</strong><br/>Total flow: ${total}`);
        })
...
function create_pie(data) {
    ...
    pie_data.forEach(({ year, conditionCounts }) => {
         ...
         // ceate a container for each pie chart with a title
        ...
        const arc = d3.arc()
         ...
         // track selected slices
    const selectedSlices = new Set();

    // Tooltip div (reuse or create if not present)
    ...
    svg.selectAll("path")
        ...
        .on("click", function(event, d) {
            // toggle selection
            if (selectedSlices.has(d.data.label)) {
                selectedSlices.delete(d.data.label);
                d3.select(this).attr("stroke", "#fff").attr("stroke-width", 1);
            } else {
                selectedSlices.add(d.data.label);
                d3.select(this).attr("stroke", "#222").attr("stroke-width", 3);
            }

            // highlight all selected
            svg.selectAll("path")
                .attr("stroke", d2 => selectedSlices.has(d2.data.label) ? "#222" : "#fff")
                .attr("stroke-width", d2 => selectedSlices.has(d2.data.label) ? 3 : 1);

            // calculate total for selected slices
            const total = pie_chart_data
                .filter(slice => selectedSlices.has(slice.label))
                .reduce((sum, slice) => sum + slice.value, 0);

            // show tooltip if any selected, otherwise hide
            if (selectedSlices.size > 0) {
                tooltip
                    .style("display", "block")
                    .html(`<strong>${year}</strong><br/>Total Students: ${total}`);
            } else {
        ...
        .on("mouseout", function() {
            // Optionally hide tooltip if nothing is selected
            if (selectedSlices.size === 0) {
                tooltip.style("display", "none");
            }
        });
